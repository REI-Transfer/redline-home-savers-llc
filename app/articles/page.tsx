...
export const metadata = {
  title: `Articles for Homeowners | ${config.companyName}`,
  description:
    ...
    <div className="bg-white text-[#1a1a1a]">
      <main className="mx-auto max-w-[760px] px-6 pt-12 pb-24">
        ...
        <div className="grid grid-cols-1 gap-5">
          {ARTICLES.map((a) => (
            <Link
              key={a.slug}
              href={a.slug}
              className="group flex flex-col sm:flex-row overflow-hidden rounded-[14px] border border-[#e5e5e5] transition-colors hover:border-[#be1e2d] hover:bg-[#fdf3f4] no-underline"
            >
              <Image
                src={a.image}
                alt={a.title}
              ...
              <div className="p-6">
                ...
                <span className="text-[15px] font-semibold text-[#be1e2d]">Read the article →</span>
              </div>
            </Link>
          ))}
        </div>
        ...
        <footer className="mt-12 pt-6 border-t border-[#e5e5e5] text-[13px] text-[#6b6b6b] text-center">
          ...
          <p>
            <Link href="/privacy" className="text-[#be1e2d] underline">
              Privacy
            </Link>
            {" · "}
            <Link href="/terms" className="text-[#be1e2d] underline">
              Terms
            </Link>
          </p>
        </footer>
